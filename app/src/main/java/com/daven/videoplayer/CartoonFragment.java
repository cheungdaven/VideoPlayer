 ...
 */
public class CartoonFragment extends Fragment implements ViewPager.OnPageChangeListener {

    private final String TAG = this.getClass().getSimpleName();
    private static final int COUNTS = Constant.CARTOON_URLS.length;
    ...
    private void initVideoArray(Context context){
        for (int i=0; i< COUNTS;i++){
            ...
            UrlImageViewHelper.setUrlDrawable(mImageViews[i],Constant.CARTOON_URLS[i]);
        }
    }

    private void initViewPager(View v){
        ViewPager viewPager = (ViewPager) v.findViewById(R.id.online_videos_cartoon);
    ...
    private void initAllTvs(View v){
        mRecylerView = (RecyclerView) v.findViewById(R.id.online_video_cartoon_all);
        ...
        mOnlineVideoAdapter = new OnlineVideoAdapter(mContext,Constant.CARTOON_ALL_URLS,null,Constant.CARTOON_ALL_NAME);
    ...
    private void initDotsView(View v){
        mViewGroup = (ViewGroup) v.findViewById(R.id.viewpager_tag_viewgroup_cartoon);
        for( int i = 0; i < COUNTS; i++){
